...
/**
 * Idempotent schema patch-up for columns/tables that were applied by hand on
 * Railway (bypassing `prisma migrate deploy`) rather than through a proper
 * migration file. Safe to run on every cold start — every statement is
 * IF NOT EXISTS / catches duplicate-object errors. On a fresh database
 * created via `prisma migrate deploy` (the whole migrations/ history) this
 * should be a no-op; kept as a safety net in case any of that drift was
...
export async function seedCmsPages(prisma: PrismaService) {
  try {
