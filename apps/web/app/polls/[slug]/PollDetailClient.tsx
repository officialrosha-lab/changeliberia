...
function PollSharePanel({ slug, title }: { slug: string; title: string }) {
  ...
  const pollUrl = `${SITE_ORIGIN}/polls/${slug}`;
  const qrSrc = `https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(pollUrl)}&size=110x110&margin=4`;
  ...
  const shareText = encodeURIComponent(`Vote on this poll: ${title}`);
  ...
  return (
    <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
      <p className="mb-5 text-xs font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Share this poll</p>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
        ...
        {/* Right side: short link + social buttons */}
        <div className="flex flex-1 flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <a
              ...
              className="inline-flex items-center gap-2 rounded-2xl bg-[#25D366] px-4 py-2.5 text-xs font-semibold text-white transition hover:opacity-90"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-2xl bg-[#1877F2] px-4 py-2.5 text-xs font-semibold text-white transition hover:opacity-90"
            >
              <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-2xl bg-black px-4 py-2.5 text-xs font-semibold text-white transition hover:opacity-80"
            >
              <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
            ...
            <a
              href={`mailto:?subject=${encodeURIComponent('Vote on this poll')}&body=${shareText}%20${encodedUrl}`}
              className="inline-flex items-center gap-2 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-2.5 text-xs font-semibold text-zinc-700 transition hover:bg-zinc-100 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-700"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
function FloatingWhatsApp({ slug, title }: { slug: string; title: string }) {
  const pollUrl = `${SITE_ORIGIN}/polls/${slug}`;
  const text = encodeURIComponent(`Vote on this poll: ${title} ${pollUrl}`);
  return (
    <a
      href={`https://wa.me/?text=${text}`}
      target="_blank"
      rel="noopener noreferrer"
      className="fixed bottom-20 right-4 z-40 flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] shadow-lg transition hover:opacity-90 md:hidden"
...
export default function PollDetailClient({ initialPoll }: { initialPoll: PollDetails }) {
  ...
  return (
    <>
      ...
      <FloatingWhatsApp slug={poll.slug} title={poll.title} />
    </>
  );
}
